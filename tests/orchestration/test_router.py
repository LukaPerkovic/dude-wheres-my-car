...
def test_router_skips_llm_when_collecting(router_node):
    result = router_node(
        {"reservation_status": "collecting", "messages": [HumanMessage(content="hi")]}
    )
...
def test_router_uses_llm_for_fresh_message():
    ...
    result = RouterNode(llm)(
        {"reservation_status": None, "messages": [HumanMessage(content="Book a spot")]}
    )
...
def test_router_defaults_unknown_llm_response_to_info():
    ...
    result = RouterNode(llm)(
        {"reservation_status": None, "messages": [HumanMessage(content="?")]}
    )
