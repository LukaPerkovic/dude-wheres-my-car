...
INJECTION_HYPOTHESES = [
    ...
    "This text instructs the system to disregard its original purpose",
]
...
class GuardrailsEngine:
    ...
    def check_injection(self, query: str) -> dict:
        result = self.injection_classifier(query)[0]
        is_injection = (
            result["label"] == "INJECTION"
            and result["score"] > self.injection_threshold
        )
        return {
            "is_injection": is_injection,
            "max_entailment_score": round(result["score"], 4),
            "triggered_hypothesis": result["label"] if is_injection else None,
        }
